...
class CreateSuppressionRequest(BaseModel):
    ...
    """ # noqa: E501
    hard_bounce: Optional[List[CreateSuppressionRequestHardBounceInner]] = Field(default=None, description="list of email addresses which you want to mark in hardBounce suppression list", alias="hardBounce")
    manual: Optional[List[CreateSuppressionRequestManualInner]] = Field(default=None, description="list of email addresses which you want to mark in manual suppression list")
    unsubscribe: Optional[List[CreateSuppressionRequestUnsubscribeInner]] = Field(default=None, description="list of email addresses which you want to mark in unsubscribe suppression list")
    spam_complaint: Optional[List[CreateSuppressionRequestSpamComplaintInner]] = Field(default=None, description="list of email addresses which you want to mark in spamComplaint suppression list", alias="spamComplaint")
    __properties: ClassVar[List[str]] = ["hardBounce", "manual", "unsubscribe", "spamComplaint"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in hard_bounce (list)
        _items = []
        if self.hard_bounce:
            for _item_hard_bounce in self.hard_bounce:
                if _item_hard_bounce:
                    _items.append(_item_hard_bounce.to_dict())
            _dict['hardBounce'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in manual (list)
        _items = []
        if self.manual:
            for _item_manual in self.manual:
                if _item_manual:
                    _items.append(_item_manual.to_dict())
            _dict['manual'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in unsubscribe (list)
        _items = []
        if self.unsubscribe:
            for _item_unsubscribe in self.unsubscribe:
                if _item_unsubscribe:
                    _items.append(_item_unsubscribe.to_dict())
            _dict['unsubscribe'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in spam_complaint (list)
        _items = []
        if self.spam_complaint:
            for _item_spam_complaint in self.spam_complaint:
                if _item_spam_complaint:
                    _items.append(_item_spam_complaint.to_dict())
            _dict['spamComplaint'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "hardBounce": [CreateSuppressionRequestHardBounceInner.from_dict(_item) for _item in obj["hardBounce"]] if obj.get("hardBounce") is not None else None,
            "manual": [CreateSuppressionRequestManualInner.from_dict(_item) for _item in obj["manual"]] if obj.get("manual") is not None else None,
            "unsubscribe": [CreateSuppressionRequestUnsubscribeInner.from_dict(_item) for _item in obj["unsubscribe"]] if obj.get("unsubscribe") is not None else None,
            "spamComplaint": [CreateSuppressionRequestSpamComplaintInner.from_dict(_item) for _item in obj["spamComplaint"]] if obj.get("spamComplaint") is not None else None
        })
        return _obj
