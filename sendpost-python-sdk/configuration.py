...
AuthSettings = TypedDict(
    "AuthSettings",
    {
        "accountAuth": APIKeyAuthSetting,
        "subAccountAuth": APIKeyAuthSetting,
    },
    total=False,
)
...
conf = sendpost-python-sdk.Configuration(
    ...
    def __init__(
        ...
        *,
        ...
        """Logging Settings
        """
        self.logger["package_logger"] = logging.getLogger("sendpost-python-sdk")
    ...
    def auth_settings(self)-> AuthSettings:
        ...
        """
        auth: AuthSettings = {}
        if 'accountAuth' in self.api_key:
            auth['accountAuth'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'X-Account-ApiKey',
                'value': self.get_api_key_with_prefix(
                    'accountAuth',
                ),
            }
        if 'subAccountAuth' in self.api_key:
            auth['subAccountAuth'] = {
                'type': 'api_key',
                'in': 'header',
                'key': 'X-SubAccount-ApiKey',
                'value': self.get_api_key_with_prefix(
                    'subAccountAuth',
                ),
            }
